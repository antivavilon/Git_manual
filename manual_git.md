...
ы## !
*(Перед тем как сделать эту команду, нужно убедиться, что HEAD и master находятся в одном commit. Для этого нужно выполнить ___git checkout masrer___ . Если мы находимся на ветке мастер, то эта команда перенгосит нас на самое актуальное состояние готовности  )*
____
...
___
### ***git branch -M main***. C помощью этой команды мы можем поменять имя текущей ветки. К примеру в этом случае мы поменяли имя ветки master на main. Новое имя ветки указывается через пробел после буквы M.
___
___
...
* Заходим на Github и создаем новый репозиторий. Копируем *ссылку на реп, на github.com* и идем в git.
* Вводим в терминале последовательно три команды:

### ***git remote add origin*** *( ссылка на реп. на github.com)*
...
### ***git clone*** *add origin (копируем ссылку на реп, на github.com )*.Проект скопирован и готов к работе.
* Идем вниз сообщения и в строчке, где у нас указан путь (в конце строки) пишем  команду ***cd*** (название реп. на github.com, ведь команда git clone создала новую папку с идентичным( названию репозитория на github.com. Расширение .git при этом не нужно) . Теперь git будет обращаться к паке, которую мы скачали с Github, а не к изначальной папке.Теперь мы можем полнорценно работать на локальном компьютере с этим проектом.
...
* Возвращаемся в наш аккаунт на github.com и видем новый репозиторий. Находим кнопку***CODE***. Нажимаем на нее у нас открывается ***строка с адресом к репозиторию***. Мы копируем строку и идем в git, на локальный компьютер.

###***git clone*** *add origin (***строка с адресом к репозиторию*** на github.com )*. И меняем в последнй строке название папки в конце на название склонированного репозитория (именно так будет называться появившаяся пака) с помощью команды ***cd***. Тепепь git будет обращаться именно в склонированный рнпозиторий, а не в изначальнуюпапку. Проект готов к работе.

* Обычно, когда хотят предложить подобного рода изменения, то всегда после основной работы над кодом проекта делают файл *readme.txt* с описанgием своих действий. Для этого созадим новую ветку с именем "discriotion*
###***git branch*** *Discription*

* Убедившись, что мы находимся на ветке"description", мы создаем файл с описанием *readme.txt*. В тексте описываем проект и свои изменения. Сохраняем иззменеия CTRL+S. И делаем команду.
###***git status***

* Теперь добавляем наш файил в отслеживание
###***git add*** *наш файл с изменениями*

* Делаем комит с описанием
###***git commit*** *-m "Комментарий"*

* Так как ветки *discription* в этом проекте до этого не было, а она должна попасть обратно на наш репозиторий на github.com, а затем и к конечному получателю, то мы выполняем следующую команду. Комаgнда подразумивает, что мы пушим находясь в этой же ветке, а не в основной.
###***git push --set-upstream origin description *** 

___
## Некоторые полезные советы для раюоты на github.com
1. Как удалить репозиторий на github.com
* Заходим в **settings**. Кнопка находится в горизонтальном меню сверху.
* Внутри **settings** идем в самый низ . Видим кнопку **delete this   repository** и нажимаем на нее.
* Внутри  **delete this   repository**  в появившемся окне нам нужно написать **имя аккаунта на github.com/имя репозитория* 

2. Как удалить отдельную ветку в репозитории на github.com
* Заходим в нужный репозиторий на github.com. Ищеи кнопку (активную надпись) **branchs**. Заходим в нее.
* Внутри **branchs** иы увидим в столбмк все имеющиеся ветки. Справа от ппнужной нам ветки иы увидим значок/иконку редактирова и рядом **удалить**. 
